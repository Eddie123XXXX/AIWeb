...
- **后端对接**：后端通过 `backend/.env` 中的 `*_HOST`、`*_PORT` 等连接上述服务；新库初始化只需要 `python -m db.run_schema`。
- **技术用途**：PostgreSQL 存用户/会话/消息/记忆/文档与切片；Redis 做会话与缓存；MinIO 存上传文件与 RAG 图片；Milvus 存记忆与 RAG 向量；RabbitMQ/Elasticsearch 当前更偏扩展或调试能力，不是主链路刚需。
...
| 模式 | 适合场景 | 说明 |
|------|----------|------|
| 最小开发链路 | 只想快速跑通聊天 / 记忆 / RAG | 至少需要 PostgreSQL、Redis、MinIO、Milvus；其余面板和增强服务可以按需忽略 |
| 全量调试链路 | 需要可视化看数据库、向量库、缓存和搜索服务 | 启动完整 compose，并结合 pgAdmin、RedisInsight、Attu、Kibana 排查数据流 |
...
这份 compose 中 `minio` 的默认账号是：

```text
MINIO_ROOT_USER=Eddiex
MINIO_ROOT_PASSWORD=12345678
```

如果你直接使用这份 compose，请把 `backend/.env` 中的：

```env
MINIO_ACCESS_KEY=Eddiex
MINIO_SECRET_KEY=12345678
```

同步改好，否则后端会因为仍使用 `minioadmin/minioadmin` 默认值而连不上对象存储。

### 2. Attu 与后端默认端口冲突

`Attu` 当前映射的是宿主机 `8000:3000`，而后端默认也习惯运行在 `8000`。二者不能同时占用同一个宿主机端口。

可选方案：

- 保持 Attu 运行，把后端改跑 `8001` 或其他端口
- 暂时停用 Attu
- 修改 compose 里的 Attu 端口映射后再启动
...
1. 在项目根目录运行 `docker compose -f infra/docker-compose.yml up -d`
2. 复制 `backend/.env.example` 为 `backend/.env`，填好模型 Key，并把 MinIO 凭据改成 `Eddiex / 12345678`
3. 在 `backend` 目录执行 `python -m db.run_schema`
4. 启动后端与前端
5. 用 pgAdmin / RedisInsight / Attu 验证数据是否真正写入，而不是只看页面是否有响应
