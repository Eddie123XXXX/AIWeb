...
- 写入：重要性打分 + 双写 PG/Milvus
- 召回：语义 + 时间衰减 + 重要性混合精排
- 反思：阈值触发总结并去重向量
- 管理：`/api/memory/*`（列表/新增/编辑/删除）
- 接入点：聊天主链路的 prompt 组装与回合结束异步写入
...
前端提供**记忆管理**浮窗：用户可查看、新增、编辑、删除记忆；编辑时后端会重新计算 embedding 并更新 Milvus。

> 这个模块的定位不是“把每一句话都存下来”，而是把真正值得跨轮复用的事实、偏好和决策留下来。
...
flowchart TB
    ...
    subgraph 召回["混合召回"]
        Intent[意图路由\nget_intent_domains]
        Semantic[Milvus 语义粗筛]
        Meta[PostgreSQL 元数据\nimportance/last_accessed]
        Decay[时间衰减]
        Rank[S_final 精排]
        Touch[更新 last_accessed]
    end

    subgraph 反思与遗忘["反思与遗忘"]
        Reflection[反思\nLLM 总结→删 fact 向量]
        Forgetting[艾宾浩斯遗忘\ncleanup_forgotten]
    end
...
**记忆管理 HTTP API**：`backend/routers/memory.py` 提供 `GET /api/memory/list`、`POST /api/memory/create`、`PUT /api/memory/{id}`、`DELETE /api/memory/{id}`，需登录，编辑会触发重新向量化。
...
├── service.py        # 核心逻辑：打分、双写、混合召回、记忆压缩（含 list/create/update/delete 供 HTTP 使用）
...
- `general_chat` 场景下可能直接跳过记忆检索，避免把闲聊上下文硬塞进 prompt。
- 记忆写入是回合结束后的异步任务，不阻塞主聊天返回，因此“刚说完立刻检索”时可能存在短暂延迟。
- 反思类记忆不会参与普通遗忘清理，这是为了让高层总结比原始 fact 活得更久。
